...
ServiceType.prototype.fromString = function fromString(string) {
  ...
  //}
    // according to standards, a service name consist of two parts
    // first part is the service name, whereas the 2nd part is the protcol
    // and it is defined as
    // service_name.protocol.domain
    // and therefore, if the service name consists of dots, for example,
    // _service.lb._udp.
    // _service.lb should be the service name
    // and the protocol should be the 2nd last token of the '.' tokenized string
    // which makes the below code wrong
  protocol = service_tokens[service_tokens.length-2];
    //service_tokens.splice(service_tokens.length-2,1);
    // service tokens should have the protocol and domain removed
    service_tokens.splice(service_tokens.length-2,2);
    if ( service_tokens.length ) {
        service_type = service_tokens.join('.');
    }
    service_tokens.length =0;
